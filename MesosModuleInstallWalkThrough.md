...
-  [mesosphere/playa-mesos](https://github.com/mesosphere/playa-mesos) runs on virtualbox, but I found a number of issues with how the nodes are configured and ran into issues using certain frameworks, such as the [Elastic Search](https://github.com/mesos/elasticsearch) framework, because of virtualbox's networking layer. The configuration that is deployed is only a single master node; so no high availability. This project also hasn't been updated in some time.
- [minimesos](https://minimesos.org/) project deploys a configuration that is backed by docker containers so if you need to modify something like the slave nodes (in our case, we need to) this isn't going to be a good option. Like the previous method, single master and no high availability.
